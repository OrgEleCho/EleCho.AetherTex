...
float4 sample_source(int sourceIndex, float2 texcoord)
{
    ...
    float4 colorYCoCg = _input[sourceIndex].Sample(_sampler, float3(uvInTile, tileIndex));
    
    return float4(ycocg_to_rgb(float3(colorYCoCg.x, colorYCoCg.y, colorYCoCg.z)), 1);
}
