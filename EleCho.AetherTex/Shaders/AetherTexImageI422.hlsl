...
float4 sample_source(int sourceIndex, float2 texcoord)
{
    ...
    int uvXInTileTexture = xyInTile.x / 2;
    if (xyInTile.y % 2 == 1)
    {
        uvXInTileTexture += TileWidth / 2;
    }
    
    int uYInTileTexture = TileHeight + xyInTile.y / 2;
    int vYInTileTexture = TileHeight + TileHeight / 2 + xyInTile.y / 2;
    
    float y = load_in_tile(tiles, tileTextureSize, 0, yXyInTileTexture).r;
    float u = load_in_tile(tiles, tileTextureSize, 0, int2(uvXInTileTexture, uYInTileTexture)).r - 0.5;
    float v = load_in_tile(tiles, tileTextureSize, 0, int2(uvXInTileTexture, vYInTileTexture)).r - 0.5;
