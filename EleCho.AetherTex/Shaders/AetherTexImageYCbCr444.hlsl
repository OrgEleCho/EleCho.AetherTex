...
float4 sample_source(int sourceIndex, float2 texcoord)
{
    ...
    float4 colorYCbCr = _input[sourceIndex].Sample(_sampler, float3(uvInTile, tileIndex));
    
    return float4(yuv_to_rgb(float3(colorYCbCr.x, colorYCbCr.y - 0.5, colorYCbCr.z - 0.5)), 1);
}
