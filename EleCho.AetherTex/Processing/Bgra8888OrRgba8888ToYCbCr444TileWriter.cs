...
{
    internal class Bgra8888OrRgba8888ToYCbCr444TileWriter : ITileWriter
    {
        private bool _disposedValue;
        private readonly ComPtr<ID3D11Device> _device;
        private readonly ComPtr<ID3D11DeviceContext> _deviceContext;
        private readonly PixelProcessor _ycbcrProcessor;

        public TextureFormat Source { get; }

        public TextureFormat Target => TextureFormat.YCbCr444;

        public Bgra8888OrRgba8888ToYCbCr444TileWriter(ComPtr<ID3D11Device> device, ComPtr<ID3D11DeviceContext> deviceContext, TextureFormat source)
        {
            _device = device;
            _deviceContext = deviceContext;
            _ycbcrProcessor = new PixelProcessor(device, deviceContext, "color",
                "0.299 * color.r + 0.587 * color.g + 0.114 * color.b, -0.169 * color.r - 0.331 * color.g + 0.5 * color.b + 0.5, 0.5 * color.r - 0.419 * color.g - 0.081 * color.b + 0.5, 1");
        ...
        public void WriteTile(TextureData textureData, ComPtr<ID3D11Texture2D> tileTexture1, ComPtr<ID3D11Texture2D>? tileTexture2, uint subResource)
        {
            ...
            }))
            {
                ...
                _ycbcrProcessor.Process(shaderResourceView, rt, textureData.Width, textureData.Height);
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_disposedValue)
            {
                _ycbcrProcessor.Dispose();
                _disposedValue = true;
            }
        }

        ~Bgra8888OrRgba8888ToYCbCr444TileWriter()
        {
